...
import { listFeedbackContributors, voteFeedbackItem } from "../../../../lib/feedback";
...
export const POST: APIRoute = async (context) => {
  try {
    ...
    const item = await voteFeedbackItem(context.locals, context.locals.user, feedbackId, payload.value);
    const contributors = await listFeedbackContributors(context.locals);

    return json({ contributors, item });
  } catch (error) {
